...
class FinancialModelingPrepMarketDataProvider(MarketDataProvider):
    ...
    async def get_candles(self, pair: str, timeframe: str, limit: int = 200) -> list[Candle]:
        ...
        params = {"symbol": self._map_symbol(pair), "apikey": self.api_key}
    ...
    def _parse_candles(
        ...
        for row in payload:
            if not isinstance(row, dict) or not row.get("date"):
                continue
            try:
                timestamp = datetime.fromisoformat(str(row["date"])).replace(tzinfo=UTC)
                candles.append(
                    Candle(
                        pair=pair,
                        timeframe=timeframe,
